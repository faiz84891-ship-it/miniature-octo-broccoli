(async () => {
    ...
    const SPAWN_BASE_DELAY_MS = 60;
    const SPAWN_JITTER_MS = 120;
    const MAX_BOTS_PER_SESSION = 96;
    ...
    function queueBotSpawn(session, hash, botName) {
        if (session.nextBotId >= MAX_BOTS_PER_SESSION) {
            console.log(`Session at max bot cap (${MAX_BOTS_PER_SESSION})`);
            return;
        }
